          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                {loadingAudio
                  ? "Carregando..."
                  : `${capitulos.length} capítulo${capitulos.length !== 1 ? "s" : ""} disponíve${capitulos.length !== 1 ? "is" : "l"}`}
              </p>
            </div>
            {bookOpen ? (
              <ChevronLeft className="h-5 w-5 text-gold flex-shrink-0 rotate-90" />
            ) : (
            ...
            <div className="mt-6 space-y-3">
              ...
              {!loadingAudio && loadError && (
                <p className="text-sm text-destructive text-center py-6">
                  Erro ao carregar capítulos: {loadError}
                </p>
              )}
              ...
              {capitulos.map((cap) => (
                <div key={cap.id}>
                  <div
                    ...
                    onClick={() => setSelected((s) => (s?.id === cap.id ? null : cap))}
                  >
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {cap.admin_only && (
                        <span className="text-xs font-semibold bg-muted/40 text-muted-foreground border border-border/40 px-2 py-0.5 rounded-full flex items-center gap-1">
